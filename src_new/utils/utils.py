...
class Pipeline():
    ...
    def launch_elasticsearch(self, launch: bool = False):
        if launch:
            logging.info("Starting Elasticsearch ...")
            status = subprocess.run(
                ['docker run -d -p 9200:9200 --name "hera" -e "discovery.type=single-node" elasticsearch:7.6.2'], shell=True
                )
            time.sleep(30)
        else:
            logging.info("Starting Elasticsearch ...")
        ...
        dicts = convert_files_to_dicts(dir_path=self.data_path, clean_func=clean_website_text, split_paragraphs=True)
        try:
